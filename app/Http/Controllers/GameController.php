...
class GameController extends Controller
{
    ...
    public function narrate(string $gameCode, Request $request, OpenAiServiceContract $openAiService, GameServiceContract $gameService)
    {
        ...
        [
            'prompt' => $prompt,
            'penalty' => $penalty
        ] = json_decode($openAiService->getJsonResponse(config('services.openai.player.filter') . "Here is the prompt by $name: '$promptAttempt'", config('services.openai.player.filter_text'))[0], true);
        ...
        if (key_exists('combat', $game->players[$user->getKey()]) && ! empty($game->players[$user->getKey()]['combat'])) {
            ...
            [
                ...
                'playerWon' => $playerWon
            ] = json_decode($openAiService->getJsonResponse(config('services.openai.player.combat') . " Player triggered a event like so: {$quest['trigger']}. Against these attackers: " . json_encode($attackers) . ". Character Data: $characterData Player luck: $luck. Player penalty: $penalty. This is what happened so far: $originalResult. Player is trying to do this: $prompt", config('services.openai.player.combat_text'))[0], true);
        ...
        [
            ...
            'lootLost' => $lootLost,
            'event' => $event,
        ] = json_decode($openAiService->getJsonResponse(config('services.openai.player.result') . " Player prompt: $prompt. Player luck: $luck. Player penalty: $penalty. Location details: {$location['description']}. Player is regular human. Player inventory is empty. Quest data " . json_encode($quests) , config('services.openai.player.result_text'))[0], true);
        ...
        if ($event) {
            ...
            ] = json_decode($openAiService->getJsonResponse(config('services.openai.character.enemy') . " {$quest['description']}. Create $totalEnemies characters. Character levels are " .  json_encode($characterLevels) . " respectively. Min level is 1 and max level is 10." , config('services.openai.characters.text'))[0], true);

            for ($i = 0; $i < $totalEnemies; $i++) {
                $characters[$i]['level'] = $characterLevels[$i];

                $weapon = json_decode($openAiService->getJsonResponse(config('services.openai.equipment.weapon')[$weaponLevels[$i] - 1] . ". Weapon is for {$characters[$i]['name']}, {$characters[$i]['description']}.", config('services.openai.equipment.text'))[0], true);
...
//        event(StoryUpdated::broadcast($game));
    ...
    public function store(Request $request, OpenAiServiceContract $openAiService)
    {
        ...
        $data = json_decode($openAiService->getJsonResponse($prompt, config('services.openai.story.text'))[0], true);
